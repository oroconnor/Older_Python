...
df['Month/Year'] = df["date_dt"].apply(lambda x: "%d/%d" % (x.month, x.year))

monthdf = df.resample('M').sum()
# monthdf = (df.groupby(['Month/Year']).size())
# monthdf.columns = ["Month/Year","NumberOfEvictions"]
print(monthdf)
print(monthdf.info())

#%%
fig, ax = plt.subplots()
ax.plot(monthdf.index, monthdf["Counts"])
ax.set_ylabel('Evictions per Month')
ax.set_xlabel("Months")
plt.suptitle("Kingston Evictions")
fig.savefig("evictions1.jpg", bbox_inches = "tight")


#%%

yeardf = df.resample('A').sum()
print(yeardf)
print(yeardf.info())
...
fig, ax = plt.subplots()
ax.plot(yeardf.index, yeardf["Counts"])
ax.set_ylabel('Number of Evictions')
ax.set_xlabel("Year")
plt.suptitle("Kingston Evictions")
fig.savefig("evictions2.jpg", bbox_inches = "tight")
